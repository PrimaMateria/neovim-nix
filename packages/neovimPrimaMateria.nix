{pkgs, neovim, noneckpain-src, telescope-recent-files-src} : let
    ...
    telescope-recent-files = import ./vimPlugins/telescopeRecentFiles.nix { src = telescope-recent-files-src; };
  ...
  neovimPrimaMateriaUnwrapped = pkgs.wrapNeovim neovim.packages.x86_64-linux.neovim {
          withNodeJs = true;
          configure = {
            customRC = import ../config { inherit ultisnipsSnippets luaConfigs; };
            packages.myVimPackage.start = plugins pkgs ++ [
              (telescope-recent-files pkgs)
              (noneckpain pkgs)
            ];
          };
        };
